...
- `PUT /api/state`: guarda la base de datos y conserva los hashes existentes en el servidor. Requiere `Authorization: Bearer <token>`.

El backend agrega cabeceras de seguridad, bloquea origenes externos, limita archivos estaticos a la web publica y evita exponer hashes por la API. Las contrasenas reales no deben guardarse en archivos publicos del repositorio.
